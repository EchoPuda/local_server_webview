...
class LocalServerDownloadService {
  ...
  /// 预载assets资源
  preloadAssetsData(List<DownloadServiceTotalAssetItem> assets) => _preloadAssetsData(assets);

  /// 预载basics资源, [basicsCache] 是旧的需要去重的缓存
  preloadBasicsData(Map basicJson, Map<String, Map> basics, List<DownloadServiceItem>? basicsCache) => _preloadBasicsData(basicJson, basics, basicsCache);
  ...
  void _downloadInQueue() {
    ...
    _log('bucket个数: ${_assetsBucket.length}  当前队列未下载个数: ${_loadQueue.length} 已下载个数：$sucCount');
    for (var queueItem in dequeueList) {
      // 判断路径之前是否存在
      if (queueItem.filePath.isNotEmpty && queueItem.loadState == LoadStateType.success) {
      ...
      final zipPath = "${LocalServerConfiguration.cacheDirectory}/${(const Uuid().v4())}.zip";
  ...
  DownloadServiceItem unarchive(DownloadServiceItem item, String downPath) {
    ...
    try {
      ...
      _log('unarchive 解压失败 item: ${item.toJson().toString()}  downPath: $downPath  err: $e');
      rethrow;
    }
  }
  ...
  List<DownloadServiceItem> _createTmpQueueList() {
    ...
    void _addQueue(DownloadServiceItem item) {
      if (dequeueList.length >= downloadMaxCount || (dequeueList.length + _loadQueue.length) >= downloadMaxCount) {
        return;
      }
      if (_loadQueue.indexWhere((element) => element.zipUrl == item.zipUrl) != -1) {
        // 下载列表中已存在则跳过
        return;
      }
  ...
  /// basics 是另外下载的处理
  void _fetchBasicsDataFromNetwork(String key, DownloadServiceItem curItem, List<DownloadServiceItem> basicsCache) {
    ...
    void checkBasicDownloadAllDown() async {
      int sucLen = basicsBucket
          ...
          .toList().length;
      if (sucLen >= (LocalServerCacheBinderSetting.instance.lsClientConfig?.basics.length ?? 1)) {
        // 保存记录
    ...
    bool checkBasicCacheIsAvailable() {
      var cacheItemList = basicsCache.where((element) => element.h5Path == key).toList();
      ...
      if (cacheItem.zipUrl == curItem.zipUrl) {
        ...
        for (var filePath in cacheItem.filePath) {
          String tmpPath = LocalServerConfiguration.joinZipPathSync(cacheItem.zipUrl, filePath);
        ...
        for (var deletePath in cacheItem.filePath) {
          String filePath = LocalServerConfiguration.joinZipPathSync(cacheItem.zipUrl, deletePath);
    ...
    Dio().download(LocalServerConfiguration.downloadUrl(curItem.zipUrl), zipPath).then((resp) {
      if (resp.statusCode != 200) {
  ...
  Map<String, Set<DownloadServiceItem>> _convertToBucketList(List<DownloadServiceTotalAssetItem> assets) {
  ...
  void _preloadAssetsData(List<DownloadServiceTotalAssetItem> assets) {
    ...
    _assetsBucket.forEach((key, value) {
      for (var element in value) {
        ...
        _log('assets bucket 已有资源 loadState: ${element.loadState} path: ${element.h5Path} zip: ${element.zipUrl}');
      }
    });
  ...
  void _preloadBasicsData(Map basicJson, Map<String, Map> basics, List<DownloadServiceItem>? basicsCache) async {
    void makeBasicItemInBucket(String key) {
      if (basicJson[key] != null) {
        DownloadServiceItem tmpItem = DownloadServiceItem()
            ..h5Path = key
            ..zipUrl = LocalServerConfiguration.downloadUrl(basicJson[key]['compress'] ?? '')
