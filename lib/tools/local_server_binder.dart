...
import 'package:local_server_for_webview/src/model/download_service_total_asset_item.dart';
...
class LocalServerCacheBinder implements LocalServerCacheBuilderProtocol {
  ...
  LocalServerClientConfig? get lsClientConfig => LocalServerCacheBinderSetting.instance.lsClientConfig;

  Map<String, dynamic>? get basicCache => LocalServerCacheBinderSetting.instance.basicCache;

  /// 当前webview是否被禁止使用server
  bool get unSupportLocalServer => LocalServerCacheBinderSetting.instance.unSupportLocalServer;
    ...
    void _fetchMemorySources(DownloadServiceTotalAssetItem tmpAsset) {
      for (var element in tmpAsset.assets) {
        for (var filePath in element.filePath) {
          String path = LocalServerConfiguration.joinZipPathSync(element.zipUrl, filePath);
