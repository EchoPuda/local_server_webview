...
import 'package:local_server_for_webview/src/model/local_server_client_config_item.dart';
...
class LocalServerConfigCache {
  ...
  static Future<List<LocalServerClientConfigItem>?> getOptions() async {
    ...
    List<Map<String, dynamic>> decode = res.cast<Map<String, dynamic>>().toList();
    var optionList = decode.map((e) => LocalServerClientConfigItem.fromJson(e)).toList();
    return optionList;
  }
  ...
  static Future setAssets(Map<String, DownloadServiceTotalAssetItem> map) async {
    Map<String, String> saveJson = map.map((key, value) => MapEntry(key, json.encode(value.toJson())));
  ...
  static Future<Map<String, DownloadServiceTotalAssetItem>?> getAssets() async {
    ...
    try {
      ...
      var res = saveJson.map((key, value) => MapEntry(key, DownloadServiceTotalAssetItem.fromJson(json.decode(value))));
      return res;
    } catch(e) {
      _log(e.toString());
      rethrow;
