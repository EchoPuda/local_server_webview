...
import 'package:local_server_for_webview/src/model/download_service_item.dart';
import 'package:local_server_for_webview/src/model/local_server_client_config.dart';
import 'package:local_server_for_webview/src/services/local_server_configuration.dart';
import 'package:local_server_for_webview/src/services/local_server_service.dart';
import 'package:local_server_for_webview/tools/local_server_binder.dart';
import 'package:local_server_for_webview/tools/local_server_config_cache.dart';
import 'package:local_server_for_webview/tools/local_server_downloader.dart';
import 'package:local_server_for_webview/tools/local_server_status_handler.dart';
...
class LocalServerClientManager implements LocalServerStatusHandler,
  ...
  void startLocalServer([String? address, int? port]) => LocalServerService.instance.startServer(address, port);
  ...
  void resignBuilder(LocalServerCacheBinder builder) => builders.remove(builder);
  ...
  fetchRespondsSources(String path) {
    ...
    if (res == null) {
      ...
      if (sourceMsg == null) {
        ...
        String path = LocalServerConfiguration.joinZipPathSync(sourceMsg['zipUrl'], sourceMsg['filePath']);
  ...
  basicsDidFullyDownloaded(bool isSuc, List<DownloadServiceItem> basicsBucket) {
    ...
    for (var loadItem in basicsBucket) {
      for (var p in loadItem.filePath) {
        String path = LocalServerConfiguration.joinZipPathSync(loadItem.zipUrl, p);
    ...
    _log('结束Basic下载，用时: ${(DateTime.now().millisecondsSinceEpoch - loadStartTime) / 1000}');
  }
  ...
  assetsDidFullyDownloaded() {
    ...
    _log('结束Asset 下载， 用时：${(DateTime.now().millisecondsSinceEpoch - loadStartTime) / 1000}');
